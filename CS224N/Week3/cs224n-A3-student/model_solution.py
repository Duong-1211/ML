...
class CausalAttention(nn.Module):
    ...
    def forward(
        ...
        attention_mask: Optional[Int[Tensor, "batch seq_len"]] = None,
        ...
        if attention_mask is not None:
            ...
            query_mask = attention_mask[:, None, :seq_len, None].to(torch.bool)
            attn_scores = attn_scores.masked_fill(~key_mask, float("-inf"))

            # Avoid NaNs for padded query rows by giving them a finite score before softmax.
        ...
        if attention_mask is not None:
            output = output * attention_mask[:, :seq_len, None].to(output.dtype)
